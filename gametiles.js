...
for (var i = 0; i < buttons.length; i++) {buttons[i].addEventListener("click", changeCanvas);}


function changeCanvas(e) {
  var buttonStr = e.target.id;
  if (buttonStr == "minus") {n++;}
  if (buttonStr == "plus") {n--;}
  if (buttonStr == "west") {istart--;}
  if (buttonStr == "east") {istart++;}
  if (buttonStr == "north") {jstart--;}
  if (buttonStr == "south") {jstart++;}
